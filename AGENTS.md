...
- Every component under `src/components` needs a `*.test.tsx`.
- Patterns under `src/patterns` are illustrative composition examples (how to assemble components into a finished UI), not shippable components, and do not require tests or axe coverage. They are still a11y-scanned through their stories in the Storybook test job.
...
- The public API of each published package is snapshotted in `packages/*/etc/*.api.md` (generated by api-extractor).
- When you add or modify a component — anything that changes exported components, props, or types — regenerate and commit the report:
  - `pnpm build && pnpm api:update`, then commit the updated `packages/*/etc/*.api.md`.
- `pnpm api:check` runs in CI and fails when the committed report is out of date, so an intentional API change must include its report update in the same change.
...
- `packages/tokens/src/tokens.css` is the single source of truth. The JS token export `packages/tokens/src/tokens.ts` is generated from it — never hand-edit it. After changing `tokens.css`, run `pnpm --filter @dds/emerald-tokens generate` and commit the result; `pnpm tokens:check` enforces this in CI.
